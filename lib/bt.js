...
function BT() {
     ...
     */
    this._options = {};
    this._initOptionsAttrName = 'ondblclick';
    this._initOptionsFormatIsJs = true;
    this.utils = {
...
BT.prototype = {
     ...
     *        {String} options[initOptionsAttrName] Атрибут, в который записываются опции блока.
     *                                              По умолчанию, `ondblclick`.
     *        {String} options[initOptionsAttrScheme] Схема данных для хранения опций блока.
     *                 Форматы:
    ...
    setOptions: function(options) {
        var i;
        var btOptions = this._options;
        for (i in options) {
            btOptions[i] = btOptions;
        }
        if (options.initOptionsAttrName) {
            this._initOptionsAttrName = options.initOptionsAttrName;
        }
        if (options.initOptionsAttrScheme) {
            this._initOptionsFormatIsJs = options.initOptionsAttrScheme === 'js';
        }
        return this;
    },
    ...
    processBtJson: function (btJson, blockName, ignoreContent) {
        ...
        var nodes = [{ json: btJson, arr: resultArr, index: 0, blockName: blockName, blockView: btJson.view }];
        ...
        while (node = nodes.shift()) {
            ...
            if (Array.isArray(json)) {
                ...
                if (json.elem) {
                    ...
                    blockView = json.view = json.view || blockView;
                } else if (json.block) {
                    block = json.block;
                }
                ...
                if (json.block) {
                    ...
                    if (!json._stop) {
                        ...
                        ctx.ctx = json;
                        if (!json.view) {
                            json.view = this._defaultViews[json.block];
                        }
                        var selectorPostfix = json.elem ? '__' + json.elem : '';
                        var matcherList = null;
                        if (
                            !(matcherList = matchers[json.block + (json.view ? '_' + json.view : '') +
                                selectorPostfix]) && json.view
                        ) {
        ...
        if (typeof json !== 'object') {
            ...
            var jattr,
                attrs = json._disableDataAttrGeneration || json.elem || !json.block ?
                    '' :
                    ' data-block="' + json.block + '"', jsParams;
            ...
            if (json._initOptions) {
                (jsParams = {}).options = json._initOptions;
            }

            var mixins = json.mixins;
            if (mixins && mixins.length) {
                (jsParams || (jsParams = {})).mixins = mixins;
            }

            if (jsParams) {
                var jsData = escapeAttr(JSON.stringify(jsParams));
                attrs += ' ' + (this._initOptionsAttrName) + '="' +
                    (this._initOptionsFormatIsJs ? 'return ' + jsData + ';' : jsData) + '"';
            }
